  ...
  const desktopBindingTags = useMemo(() => {
    ...
    const combined = Array.from(
      ...
      return {
        ...
        rssi: -120,
        battery: item?.battery_percent ?? null,
        counter: null,
    ...
    return boundTagIds
      .map((tagId) => {
        ...
        return {
          ...
          nickname,
          webId,
          battery: local?.battery ?? item?.battery_percent ?? null,
    ...
    return boundTagIds
      .map((tagId) => {
        ...
        return {
          ...
          rssi: local?.rssi ?? null,
          battery: local?.battery ?? overview?.battery_percent ?? null,
  ...
  const dashboardActions: DashboardScreenActions = {
    ...
    onConnectTag: (tagId) => {
      void (async () => {
        ...
        const batteryPercent = ble.tags[tagId]?.battery ?? ble.localTagLocations[tagId]?.battery ?? null;
        await backend.recordConnectedTagLastSeen(tagId, batteryPercent);
      })();
    },
    onDisconnectTag: () => {
