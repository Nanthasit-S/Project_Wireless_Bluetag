...
export function useMapSync({
    ...
    try {
      const res = await fetch(`${base}/api/tags`, {
        ...
        body: JSON.stringify({
          ...
          battery_percent: params.batteryPercent ?? null,
        }),
      });
      if (res.status === 401) {
      ...
      if (mapQueryTag === params.tagId) {
        setMapTag((current) => ({
          ...
          estimated_longitude: params.longitude,
          estimate_source: params.estimateSource,
          battery_percent: params.batteryPercent ?? current?.battery_percent ?? null,
        }));
      }
  ...
  async function recordConnectedTagLastSeen(tagId: string, batteryPercent?: number | null) {
