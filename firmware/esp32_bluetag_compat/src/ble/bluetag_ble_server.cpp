...
void BlueTagBleServer::begin() {
  ...
  if (BlueTagConfig::kBatteryAdcPin >= 0) {
    ...
    analogSetPinAttenuation(
        BlueTagConfig::kBatteryAdcPin,
        static_cast<adc_attenuation_t>(BlueTagConfig::kBatteryAdcAttenuationDb));
  }
...
uint8_t BlueTagBleServer::readBatteryPercent() const {
  return batteryPercentFromCurve(static_cast<float>(readBatteryMilliVolts()));
}
